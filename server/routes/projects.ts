...
projects.post('/', async (c) => {
    const prisma = getPrisma(c.env);
    try {
        const { name, description, status, stack, images, clientId } = await c.req.json();

        // Validate required fields
        if (!name || !description || !status) {
            return c.json({ error: 'Name, description, and status are required' }, 400);
        }
...
projects.post('/:id/assign-client', async (c) => {
    ...
    try {
        ...
        return c.json({ error: error.message || 'Failed to assign client to project' }, 500);
    }
});
...
projects.post('/:id/remove-client', async (c) => {
    ...
    try {
        ...
        return c.json({ error: error.message || 'Failed to remove client from project' }, 500);
    }
});
...
projects.post('/:id/envvars', async (c) => {
    ...
    try {
        ...
        return c.json({ error: error.message || 'Failed to add environment variable' }, 500);
    }
});
