...
export default defineEventHandler(async (event) => {
	...
	if (body.targetId === id) {
		throw createError({ statusCode: 400, statusMessage: "cannot link issue to itself" });
	}
	...
	if (!target) throw createError({ statusCode: 404, statusMessage: "target issue not found" });
