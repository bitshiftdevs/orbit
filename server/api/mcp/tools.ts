...
export async function listProjects(_args: Record<string, any>, event: H3Event) {
...
export async function getProject(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
...
export async function listIssues(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function getIssue(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function deleteIssue(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function getIssueLinks(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
  ...
  const rows = await db
    ...
    .innerJoin(
      ...
      or(
        and(
          eq(issueLinks.sourceId, args.id),
          eq(issues.id, issueLinks.targetId),
        ),
        and(
          eq(issueLinks.targetId, args.id),
    ...
    .innerJoin(projects, eq(projects.id, issues.projectId))
    .where(
      or(eq(issueLinks.sourceId, args.id), eq(issueLinks.targetId, args.id)),
    );
...
export async function addIssueLink(
  args: {
    sourceId: string;
    targetId: string;
    kind?: "blocks" | "duplicates" | "relates_to";
  },
  event: H3Event,
) {
  const user = await getToolUser(event);
  ...
  if (!src)
    throw createError({
      statusCode: 404,
      statusMessage: "source issue not found",
    });
...
export async function listSprints(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function listTeam(_args: Record<string, any>, event: H3Event) {
  const db = getDb();
...
export async function search(args: { q: string }, event: H3Event) {
...
export async function getVelocity(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function getCycleTime(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function listSecrets(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function deleteSecret(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function listEnvVars(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function revealEnvVar(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function deleteEnvVar(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function exportDotenv(
  ...
  const rows = await db
    ...
    .from(envVars)
    .where(
      and(eq(envVars.projectId, project.id), eq(envVars.scope, args.scope)),
    )
...
export async function listFiles(args: { idOrKey: string }, event: H3Event) {
  const user = await getToolUser(event);
  const project = await toolLoadProject(args.idOrKey);
  await toolAssertMember(user, project.id);
  const db = getDb();
...
export async function deleteFile(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
...
export async function getFileContent(args: { id: string }, event: H3Event) {
  const user = await getToolUser(event);
  const db = getDb();
  ...
  if (!isText)
    throw createError({
      statusCode: 400,
      statusMessage: "file is not a text file",
    });
