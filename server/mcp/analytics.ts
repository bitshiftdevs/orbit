...
import { User, sprints, issues } from "../db/schema";
    ...
    async ({ idOrKey }) => {
      ...
      const rows = await db
        .select({
          ...
          committed:
            sql<number>`coalesce(sum(${issues.storyPoints}), 0)`.mapWith(
              Number,
            ),
          completed:
  ...
  server.tool(
    ...
    async ({ idOrKey }) => {
      ...
      const rows = await db
        ...
        .from(issues)
        .where(
          and(eq(issues.projectId, project.id), isNotNull(issues.completedAt)),
        )
        .groupBy(issues.type)
        .orderBy(issues.type);
      return {
