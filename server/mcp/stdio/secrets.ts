...
export function register(server: McpServer, api: ApiFn): void {
	...
	server.tool(
		...
		async ({ idOrKey }) => {
			const data = await api<{ secrets: unknown[] }>("GET", `/projects/${idOrKey}/secrets`);
	...
	server.tool(
		...
		async ({ id, reason }) => {
			...
			const data = await api<{ value: string }>("GET", `/secrets/${id}/reveal${params}`);
	...
	server.tool(
		...
		{ idOrKey: z.string().describe("Project UUID or short key") },
		async ({ idOrKey }) => {
			const data = await api<{ envVars: unknown[] }>("GET", `/projects/${idOrKey}/env`);
			return { content: [{ type: "text", text: JSON.stringify(data.envVars, null, 2) }] };
		},
	);
	...
	server.tool(
		...
		async ({ idOrKey, scope }) => {
			const text = await fetch(
				...
				{ headers: { Authorization: `Bearer ${process.env.ORBIT_API_TOKEN ?? ""}` } },
			).then((r) => r.text());
