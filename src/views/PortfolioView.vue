	...
	return (
		projects.value
			...
			.filter((p) => {
				...
				return p.stack.some(
					(tech) => tech.toLowerCase() === selectedTech.value.toLowerCase(),
				);
			})
	);
});
...
<template>
	<div>
		...
		<div class="bg-card py-16 px-4 mb-12 rounded-xl">
			<div class="max-w-4xl mx-auto text-center">
				...
				<div class="flex flex-wrap justify-center gap-4">
					<Button as="a" :href="`mailto:${userProfile.email}`"
						class="bg-primary hover:bg-primary/90 flex items-center gap-2">
						<Mail class="w-4 h-4" /> Contact Me
					</Button>
					<Button as="a" :href="`https://github.com/${userProfile.github}`" target="_blank" rel="noopener"
		...
		<div class="mb-8">
			<div class="flex flex-col md:flex-row gap-4 mb-4">
				...
				<Select v-model="selectedTech"
					class="w-full md:w-64 bg-input border border-border rounded-md px-3 py-2 text-foreground">
					<SelectTrigger>
		...
		<!-- Projects -->
		<div>
			<div v-if="loading" class="text-muted-foreground text-center py-12">
				Loading projects...
			</div>
			<div v-else-if="error" class="text-destructive text-center py-12">
				{{ error }}
			</div>
			<div v-else>
				...
				<div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
					<Card v-for="project in filteredProjects" :key="project.id"
						class="bg-card border-border hover:border-primary transition-all overflow-hidden">
						...
						<CardContent>
							...
							<div class="flex flex-wrap gap-2">
								<span v-for="tech in project.stack" :key="tech"
									class="bg-secondary text-secondary-foreground text-xs px-2 py-1 rounded">{{ tech }}</span>
							</div>
						</CardContent>
						<CardFooter>
							<Button class="w-full bg-primary hover:bg-primary/90" @click="viewProjectDetails(project.id)">View
								Details</Button>
						</CardFooter>
					</Card>
				</div>
		...
		<div class="mt-16 bg-card rounded-xl p-8">
			...
			<div class="flex flex-wrap justify-center gap-3">
				<div v-for="skill in userProfile.skills" :key="skill"
					class="bg-secondary px-4 py-2 rounded-lg text-primary font-medium">
					{{ skill }}
				</div>
			</div>
		</div>
		...
		<div class="mt-16 text-center">
			...
			<Button as="a" :href="`mailto:${userProfile.email}`" size="lg" class="bg-primary hover:bg-primary/90">Get in
				Touch</Button>
		</div>
	</div>
</template>
