...
<template>
	<div>
		<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
			...
			<Button class="font-bold" @click="openAddClient"> + Add Client </Button>
		</div>
		<div>
			<div v-if="loading" class="text-gray-400">Loading clients...</div>
			<div v-else-if="error" class="text-red-400">{{ error }}</div>
			<div v-else>
				<div v-if="clients.length === 0" class="text-gray-400">
					No clients found.
				</div>
						...
						<CardContent>
							<div class="space-y-2">
								<div class="flex items-center gap-2">
									<Label>Email</Label>
									<Input :default-value="client.email" disabled />
								</div>
								<div v-if="client.company" class="flex items-center gap-2">
									<Label>Company</Label>
									<Input :default-value="client.company" disabled />
								</div>
								<div v-if="client.phone" class="flex items-center gap-2">
									<Label>Phone</Label>
									<Input :default-value="client.phone" disabled />
								</div>
							</div>
						</CardContent>
			...
			<DialogContent class="max-w-lg w-full bg-gray-950 text-white">
				...
				<form @submit.prevent="saveClient" class="space-y-4">
					...
					<div class="space-y-2">
						...
						<p v-if="formErrors.email" class="text-sm font-medium text-red-500">
							{{ formErrors.email }}
						</p>
					</div>
